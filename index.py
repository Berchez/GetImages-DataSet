...
for y in range(1, (numberOfPages + 1)): 
    ...
    for x in range(0, (len(content))):
        ...
        urllib.request.urlretrieve(content[x].get_attribute("src"), f'./images/{imageToBeDonwloaded}-{(imagemNum)}.jpg')
...
for z in range(0, RestOfImages):
    imagemNum = imagemNum + 1
    urllib.request.urlretrieve(content[z].get_attribute("src"), f'./images/{imageToBeDonwloaded}-{(imagemNum)}.jpg')
